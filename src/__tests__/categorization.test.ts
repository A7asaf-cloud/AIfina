...
describe('categorize() with custom rules', () => {
  it('identifies custom rules from localStorage correctly', () => {
    ...
    localStorage.setItem('fil_active_user_id', 'test_user');
    localStorage.setItem(
      'fil_u_data_test_user',
      JSON.stringify({
        customRules: {
          'ספייס אינביידרז': 'מזון ושוק',
          'random game': 'בידור',
        },
      })
    );
    ...
    // Exact match
    expect(categorize('ספייס אינביידרז').cat).toBe('מזון ושוק');
    expect(categorize('random game').cat).toBe('בידור');

    // Partial match
    expect(categorize('ספייס אינביידרז תל אביב').cat).toBe('מזון ושוק');
