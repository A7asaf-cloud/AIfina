...
export const ConnectionsTab: React.FC<Props> = ({ profile, transactions, onNavigateToTab }) => {
  ...
  return <main dir="rtl" className="mx-auto max-w-4xl space-y-5 px-4 py-7 md:px-8">
    ...
    <Card className="space-y-4"><div className="flex gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#eaf5e5] text-[#39704b]"><Building2 size={22} /></span><div><h2 className="font-bold text-ink">בנק הפועלים · בנקאות פתוחה</h2><p className="mt-1 text-sm leading-6 text-muted">החיבור משתמש במסלול PSD2 של הבנק. AIfina לא מבקשת ולא שומרת סיסמה של הבנק או חברת האשראי.</p></div></div>{CONFIG.OPEN_BANKING_CONNECT_URL ? <Button onClick={startSecureConnection}><ExternalLink size={17} />חיבור מאובטח לבנק</Button> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>ה‑Sandbox מוכן לחיבור, אך הגישה החיה עוד לא אושרה.</strong><br />אחרי אישור הבנק והגדרת כתובת האימות בשרת, הכפתור כאן יפתח את תהליך ההסכמה המאובטח.</div>}<a className="inline-flex items-center gap-2 text-sm font-bold text-primary underline-offset-4 hover:underline" href="https://poalimdev.co.il/get-started" target="_blank" rel="noreferrer">פתיחת סביבת הפיתוח של הפועלים <ExternalLink size={15} /></a><div className="flex items-center gap-2 text-xs text-muted"><LockKeyhole size={14} />הסכמה ניתנת לביטול אצל הבנק בכל עת.</div></Card>
