...
import { categorize, getCustomRules, CATEGORIES, CategoryKey } from '../utils/categories';
...
function toAppTransaction(t: ScraperTransaction): Transaction {
  ...
  const catInfo = categorize(cleanDesc);
  const finalCat = matchedCat ?? mappedCat ?? catInfo.cat;
  
  const catDetails = CATEGORIES[finalCat as CategoryKey] || CATEGORIES['שונות'];
  ...
  return {
    ...
    amount: (() => {
      ...
      const isIncome = t.amount > 0
        ...
        || desc.includes('העברה נכנס')
        || (t.type === 'credit');
      return isIncome ? Math.abs(t.amount) : -Math.abs(t.amount);
    })(),
...
export function BankScraper({ onImportTransactions, onBack }: BankScraperProps) {
  ...
  if (otpMode) {
    return (
      <div className="bg-card border border-line rounded-3xl p-6 shadow-xl space-y-5 text-right">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-[#FFF3E0] text-[#F5A623] flex items-center justify-center">
            <Lock className="w-6 h-6" />
          </div>
          <div>
            <h3 className="font-bold text-ink text-base">נדרש קוד אימות (OTP)</h3>
            <p className="text-muted text-xs mt-0.5">הבנק שלח SMS עם קוד חד-פעמי</p>
          </div>
        </div>
        ...
        <input
          ...
          className="w-full bg-surface border border-line text-ink text-center text-2xl font-mono tracking-widest rounded-xl px-4 py-4 outline-none focus:border-primary/50"
          autoFocus
        />
        ...
        <div className="flex gap-3">
          <button
            ...
            className="py-3 px-4 bg-line text-muted font-bold text-sm rounded-xl cursor-pointer"
          >
            ביטול
          </button>
          <button
            ...
            className="flex-1 py-3 bg-primary hover:bg-primary/90 text-white font-extrabold text-sm rounded-xl transition-all cursor-pointer disabled:opacity-50"
          >
      ...
      <div className="bg-card border border-line rounded-3xl p-6 shadow-xl space-y-5 text-right">
        <div className="flex items-center justify-between pb-3 border-b border-line">
          <button
            ...
            className="text-xs text-muted hover:text-ink flex items-center gap-1 font-bold cursor-pointer"
          >
            <ArrowRight className="w-4 h-4 rotate-180" />
            <span>חזור</span>
          </button>
          <h3 className="font-bold text-ink text-base">{previewLabel}</h3>
        </div>

        <div className="bg-income/10 border border-income/20 p-3 rounded-2xl text-income text-xs font-bold flex items-center justify-between">
          <span>נמצאו {previewTxs.length} עסקאות (3 חודשים אחרונים)</span>
          <CheckCircle2 className="w-5 h-5" />
        </div>

        <div className="max-h-72 overflow-y-auto divide-y divide-line bg-surface p-3 rounded-2xl border border-line">
            ...
            <div key={idx} className="py-2 flex justify-between items-center gap-3 text-xs">
              <div className="flex flex-1 items-center gap-2 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-ink font-medium truncate">{tx.description}</p>
                  <p className="text-muted truncate">{fmtDate(tx.date)} · {tx.cat}</p>
                </div>
              </div>
              <span className={`font-bold shrink-0 mr-2 ${tx.amount > 0 ? 'text-income' : 'text-ink'}`}>
                {fmtILS(tx.amount)}
              </span>
            </div>
          ))}
          {previewTxs.length > 100 && (
            <p className="text-center text-muted text-xs py-2">+ עוד {previewTxs.length - 100} עסקאות</p>
          )}
        </div>
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row">
          ...
          <button
            ...
            className="flex-1 py-3 bg-primary hover:bg-primary/90 text-white font-extrabold text-sm rounded-xl shadow-lg shadow-primary/20 cursor-pointer"
          >
    ...
    return (
      <div className="bg-card border border-line rounded-3xl p-6 shadow-xl space-y-5 text-right">
        <div className="flex items-center justify-between pb-3 border-b border-line">
          ...
          <h3 className="font-bold text-ink text-base">חיבור מוסד פיננסי חדש</h3>
        </div>

        {/* Security notice */}
        <div className="bg-surface border border-line rounded-2xl p-3 flex items-start gap-2">
        ...
        <div className="relative">
          <label className="text-xs text-muted font-medium block mb-1.5">מוסד פיננסי</label>
          <button
            onClick={() => setShowCompanyDropdown(v => !v)}
            className="w-full bg-surface border border-line text-right px-4 py-3 rounded-xl flex items-center justify-between text-sm cursor-pointer"
          >
            <span className={`min-w-0 truncate ${selectedCompany ? 'text-ink' : 'text-muted'}`}>
          ...
          {showCompanyDropdown && (
            <div className="absolute top-full mt-1 right-0 left-0 bg-card border border-line rounded-xl overflow-hidden z-40 shadow-xl max-h-56 overflow-y-auto">
              {COMPANIES.map(c => (
                <button
                  key={c.id}
                  onClick={() => {
                  ...
                  className="w-full text-right px-4 py-2.5 text-sm hover:bg-line text-ink flex items-center justify-between cursor-pointer"
                >
                  <span>{c.name}</span>
                  {c.otp && <span className="text-[10px] text-[#D4A017] bg-[#FFF8E1] px-2 py-0.5 rounded-full">OTP</span>}
                </button>
              ))}
            </div>
          )}
        ...
        {selectedCompany && (
          <>
            {/* Display name */}
            <div>
              <label className="text-xs text-muted font-medium block mb-1.5">שם תצוגה</label>
              <input
                type="text"
                value={displayName}
                onChange={e => setDisplayName(e.target.value)}
                className="w-full bg-surface border border-line text-ink text-sm px-4 py-3 rounded-xl outline-none focus:border-primary/50 text-right"
              />
            </div>
            ...
            {selectedCompany.otp && (
              <p className="text-xs text-[#B8860B] bg-[#FFF8E1] border border-[#FFE082] rounded-xl px-3 py-2">
                בנק זה ישלח קוד SMS בזמן הסריקה הראשונה.
              </p>
            )}

            {addError && (
              <div className="text-expense text-xs bg-expense/10 border border-expense/20 px-3 py-2 rounded-xl flex items-center gap-2">
            ...
            <button
              ...
              disabled={addLoading}
              className="w-full py-3.5 bg-primary hover:bg-primary/90 text-white font-extrabold text-sm rounded-xl transition-all shadow-lg shadow-primary/20 cursor-pointer disabled:opacity-60"
            >
  ...
  return (
    <div className="space-y-4 text-right">
      <div className="bg-card border border-line rounded-3xl p-6 shadow-xl space-y-5">
        {/* Header */}
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-ink text-base">ייבוא ישיר מהבנק</h3>
          </div>
        </div>

        {globalError && (
          <div className="bg-expense/10 border border-expense/20 text-expense text-xs p-3 rounded-xl flex items-start gap-2">
        ...
        {loadingList ? (
          ...
          <div className="text-center py-8 space-y-3">
            <div className="w-16 h-16 rounded-full bg-secondary/10 text-secondary flex items-center justify-center text-3xl mx-auto">
              <Building2 className="w-8 h-8" />
            </div>
            <p className="text-ink font-bold text-sm">לא מוגדרים מוסדות פיננסיים</p>
            <p className="text-muted text-xs">הוסף חיבור לבנק או כרטיס אשראי שלך</p>
          </div>
        ) : (
          <div className="space-y-3">
            {institutions.map(inst => (
              <div
                key={inst.id}
                className={`bg-surface border rounded-2xl p-4 flex items-center justify-between gap-3 transition-colors ${
                  inst.is_active ? 'border-line' : 'border-line/50 opacity-60'
                }`}
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-ink font-bold text-sm truncate">{inst.display_name}</p>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    ...
                    title="משוך עסקאות"
                    className="p-2 hover:bg-line rounded-xl text-income hover:text-income/80 transition-colors cursor-pointer disabled:opacity-40"
                  >
                    {scrapingId === inst.id
                  ...
                  <button
                    ...
                    className="p-2 hover:bg-line rounded-xl text-muted hover:text-ink transition-colors cursor-pointer"
                  >
                    {inst.is_active
                  ...
                  <button
                    ...
                    title="מחק"
                    className="p-2 hover:bg-expense/10 rounded-xl text-muted hover:text-expense transition-colors cursor-pointer"
                  >
        ...
        <button
          ...
          className="w-full py-3 border border-dashed border-secondary/30 hover:border-secondary/60 hover:bg-secondary/5 text-secondary font-bold text-sm rounded-2xl transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
      ...
      {/* Legal notice */}
      <div className="bg-card border border-line rounded-2xl p-4 flex items-start gap-3">
